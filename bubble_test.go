...
func TestBubbleStruct(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			BubbleStruct[int](tt.args.data, tt.args.direction)
			if !reflect.DeepEqual(tt.args.data, tt.want) {
				t.Errorf("Bubble() = %v, want %v", tt.args.data, tt.want)
			}
		})
...
// Benchmarks
func benchmarkBubble[V constraints.Ordered](data Slice[V], direction direction, b *testing.B) {
	for n := 0; n < b.N; n++ {
...
func BenchmarkBubbleInt(b *testing.B) {
	benchmarkBubble([]int{4, 5, 8, 7}, Asc, b)
}
func BenchmarkBubbleString(b *testing.B) {
	benchmarkBubble([]string{"c", "a", "b"}, Asc, b)
}

func BenchmarkBubbleStruct(b *testing.B) {
	benchmarks := []struct {
		name      string
		data      []Person
		direction direction
	...
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				BubbleStruct[int](bm.data, bm.direction)
			}
		})
	}
}
