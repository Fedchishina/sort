...
func TestInsertionStruct(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			InsertionStruct[int](tt.args.data, tt.args.direction)
			if !reflect.DeepEqual(tt.args.data, tt.want) {
				t.Errorf("Insertion() = %v, want %v", tt.args.data, tt.want)
			}
		})
...
// Benchmarks
func benchmarkInsertion[V constraints.Ordered](data Slice[V], direction direction, b *testing.B) {
	for n := 0; n < b.N; n++ {
...
func BenchmarkInsertionInt(b *testing.B) {
	benchmarkInsertion([]int{4, 5, 8, 7}, Asc, b)
}
func BenchmarkInsertionString(b *testing.B) {
	benchmarkInsertion([]string{"c", "a", "b"}, Asc, b)
}

func BenchmarkInsertionStruct(b *testing.B) {
	benchmarks := []struct {
		name      string
		data      []Person
		direction direction
	...
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				InsertionStruct[int](bm.data, bm.direction)
			}
		})
	}
}
